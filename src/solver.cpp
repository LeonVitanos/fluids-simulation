...
void dens_step(int N, float *x, float *x0, float *u, float *v, float diff, float dt, float *boundaries)
{
	...
	SWAP(x0, x);
	diffuse(N, 0, x, x0, diff, dt, boundaries);
	SWAP(x0, x);
	advect(N, 0, x, x0, u, v, dt, boundaries);
}

void vel_step(int N, float *u, float *v, float *u0, float *v0, float visc, float dt, float *boundaries)
{
